...
public class SunshineSyncUtils {
    ...
    private static final String FETCH_WEATHER_JOB_TAG = "fetch_weather_tag";

    private static boolean sInitialized;

    //  COMPLETED (12) Create a method to schedule our periodic weather sync
    private static void scheduleFetchWeatherData(Context context) {
        Driver driver = new GooglePlayDriver(context);
        FirebaseJobDispatcher dispatcher = new FirebaseJobDispatcher(driver);

        Job job = dispatcher.newJobBuilder()
                .setService(SunshineFirebaseJobService.class)
                .setTag(FETCH_WEATHER_JOB_TAG)
...
//      COMPLETED (13) Call the method you created to schedule a periodic weather sync
        scheduleFetchWeatherData(context);
